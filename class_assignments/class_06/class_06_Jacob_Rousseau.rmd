...
ggplot(maryland_pills_per_year) +
  ...
  labs(x="Year", y="Total pills", title="In Maryland, opioids rise in 2011", subtitle = "Total pills shipped to Maryland by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)

# It differs from Baltimore City because Baltimore City started with a rise, dropped and then evened out. Whereas for the entire state it started low and then rised and stayed somewhere in the middle in 2014. 
...
```{r}
america_population_2012 <- state_population(key = key) %>%
  clean_names() %>%
  filter(year=="2012") %>%
...
```{r}
america_population_2012_ <- america_pills_2012 %>%
  inner_join(america_population_2012, by=("buyer_state")) %>%
  select(buyer_county, dosage_unit, population)
```

```{r}
america_population_2012 <- america_pills_2012 %>%
