...
```{r}
opioid_deaths <- read_tsv("2006-2012.txt") %>%
  clean_names() %>%
...
# Group into five buckets and calculate median household income and average pills per person
 ...
 ungroup() %>%
 mutate(quintile = ntile(average_pills_per_person, 5)) %>%
 group_by(quintile) %>%
...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Median Household Income", y="Average Pills Per Person", title="", caption = "Source: DEA ARCOS database, via Washington Post")
...
```
```{r}
correlation_household_income <- pills_population_income %>%
  ungroup() %>%
  select(median_household_income, average_pills_per_person) 
...
```{r}
cor.test(correlation_household_income$median_household_income, correlation_household_income$average_pills_per_person)


```

```{r}
#The lower household income the higher amount of pills per person
