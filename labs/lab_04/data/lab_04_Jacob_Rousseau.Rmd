...
mingo %>%
  ...
  summarise(number_of_pills = sum(dosage_unit)) %>%
  arrange(desc(number_of_pills))

# The most significant combo is Actavis Pharma, Inc. to Miami-Luken to TUG VALLEY PHARMACY, LLC. The number of pills was 4,954,200
```
...
mingo %>%
  group_by(reporter_name, reporter_dea_no, reporter_city, reporter_state, reporter_address1) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit), 
            ) %>%
  arrange(reporter_name)

# They have a different DEA number and address. 
...
```{r}
# In order to group these together I would use case_when() formula and then select by the three cardinal healths. Below is my thought process... I used the tidyverse page for help 
...
# mingo %>%
  ...
  # mutate(
   #  type = case_when("CARDINAL HEALTH, CARDINAL HEALTH 110, CARDINAL HEALTH, INC." ~"cardinal_health")
  # )
