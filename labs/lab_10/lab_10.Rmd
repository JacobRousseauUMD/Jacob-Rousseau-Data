...
```{r}
opioid_deaths_county <- read_tsv("2006-2012.txt")
...
```{r}
medicare_opioids <- read_csv("medicare_opioids_dirty.csv")
...
medicare_opioids_clean <- read_csv("medicare_opioids_clean.csv")
...
```{r}
medicare_scrape_working <- medicare_opioids %>%
  clean_names() %>%
  remove_empty_rows() %>%
...
```{r}
glimpse(medicare_scrape_working)
